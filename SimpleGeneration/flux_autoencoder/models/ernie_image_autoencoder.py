"""
ERNIE-Image AutoEncoder
ERNIE-Image VAE uses the same architecture as FLUX.2 VAE (AutoencoderKLFlux2).
https://github.com/black-forest-labs/flux2/blob/main/src/flux2/autoencoder.py

ERNIE-Image VAE config:
in_channels=3, out_channels=3, block_out_channels=[128, 256, 512, 512],
layers_per_block=2, latent_channels=32, patch_size=[2, 2],
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.quant_conv = nn.Conv2d(2 * z_planes,
                                    2 * z_planes,
                                    kernel_size=1,
                                    stride=1,
                                    padding=0,
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.post_quant_conv = nn.Conv2d(z_planes,
                                         z_planes,
                                         kernel_size=1,
                                         stride=1,
                                         padding=0,
...
# ERNIE-Image VAE (same architecture as FLUX.2-dev VAE)
# inplanes=3,
# planes=128,
# planes_mult=[1, 2, 4, 4],
# res_block_nums=2,
...
class AutoEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.ps = [2, 2]
        self.bn = nn.BatchNorm2d(math.prod(self.ps) * z_planes,
                                 eps=self.bn_eps,
                                 momentum=self.bn_momentum,
                                 affine=False,
                                 track_running_stats=True)

        self.logvar = nn.Parameter(torch.full((1, ), logvar_init))
    ...
    def forward_train(self, x):
        ...
        kl_out = 0.5 * torch.mean(mean.pow(2) + logvar.exp() - 1.0 - logvar)
