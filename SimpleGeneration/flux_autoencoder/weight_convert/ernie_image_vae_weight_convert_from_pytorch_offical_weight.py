...
from SimpleGeneration.flux_autoencoder.models.ernie_image_autoencoder import AutoEncoder
...
def convert_diffusers_key_to_flux_key(diffusers_key):
    ...
    ERNIE-Image VAE uses diffusers format (AutoencoderKLFlux2 from diffusers).
    Our ernie_image_autoencoder.py uses flux-style key names.
    ...
    Top-level keys:
      quant_conv.xxx -> encoder.quant_conv.xxx
      post_quant_conv.xxx -> decoder.post_quant_conv.xxx
      bn.xxx -> bn.xxx  (unchanged)
...
if __name__ == '__main__':
    ...
    # /root/autodl-tmp/pretrained_models/ernie-image_pytorch_official_weights/vae/diffusion_pytorch_model.safetensors

    saved_model_path = '/root/autodl-tmp/pretrained_models/ernie-image_pytorch_official_weights/vae/diffusion_pytorch_model.safetensors'
    ...
    if not_loaded_model_keys:
        print('Model keys not loaded (expected for logvar):')
        for key in not_loaded_model_keys:
            print(f'  {key}')

    save_model_name = 'ernie_image_vae'
    torch.save(
        convert_dict,
        f'/root/autodl-tmp/pretrained_models/ernie_image_convert_from_pytorch_official_weights/{save_model_name}_convert_from_pytorch_official_weight.pth'
    )
