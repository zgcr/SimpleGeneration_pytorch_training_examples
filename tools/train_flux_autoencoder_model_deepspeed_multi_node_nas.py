...
def main():
    ...
    # for deepspeed==0.19.3, Muon optimizer is not yet compatible with ZeRO Stage 3,only support ZeRO stage 0/1/2.
    ...
    # Create EMA models after deepspeed.initialize().
    # For ZeRO-3, param.ds_tensor (local shard) is only available after init.
    # For ZeRO-0/1/2, params are still full after init.
    # Generator and discriminator each have their own EMA model.
    if config.use_ema_model:
        # Generator EMA: uses config.deepspeed_zero_stage
        generator_ema_model = DeepSpeedEmaModel(
            generator_model_engine,
            config,
            decay=config.generator_ema_model_decay,
            tau=config.generator_ema_model_tau)
        config.generator_ema_model = generator_ema_model
        log_info = f'Generator EMA model created with decay={config.generator_ema_model_decay}, tau={config.generator_ema_model_tau}, zero_stage={config.deepspeed_zero_stage}'
        logger.info(log_info) if local_rank == 0 and total_rank == 0 else None

        # Discriminator EMA: always uses DISCRIMINATOR_ZERO_STAGE (0).
        # DeepSpeedEmaModel reads config.deepspeed_zero_stage, so temporarily
        # override it for the discriminator EMA creation.
        original_zero_stage = config.deepspeed_zero_stage
        config.deepspeed_zero_stage = DISCRIMINATOR_ZERO_STAGE
        discriminator_ema_model = DeepSpeedEmaModel(
            discriminator_model_engine,
            config,
            decay=config.discriminator_ema_model_decay,
            tau=config.discriminator_ema_model_tau)
        config.deepspeed_zero_stage = original_zero_stage
        config.discriminator_ema_model = discriminator_ema_model
        log_info = f'Discriminator EMA model created with decay={config.discriminator_ema_model_decay}, tau={config.discriminator_ema_model_tau}, zero_stage={DISCRIMINATOR_ZERO_STAGE}'
        logger.info(log_info) if local_rank == 0 and total_rank == 0 else None
    ...
    if os.path.exists(resume_generator_model) and os.path.exists(
        ...
        if client_state is not None:
            ...
            if config.use_ema_model:
                if 'generator_ema_model_state' in client_state:
                    config.generator_ema_model.load_state_dict(
                        client_state['generator_ema_model_state'])
                if 'discriminator_ema_model_state' in client_state:
                    config.discriminator_ema_model.load_state_dict(
                        client_state['discriminator_ema_model_state'])
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        if need_save_epoch or need_save_best:
            if config.use_ema_model:
                # EMA enabled: save EMA model weights, skip training model
                # For generator: For ZeRO-3, get_ema_model_state_dict uses
                # all_gather (collective op), so all ranks must call it.
                ...
                # For discriminator: always ZeRO Stage 0, EMA state_dict is
                # full on every rank, only rank 0 saves.
        ...
        # Include EMA states in client_state for resume
        if config.use_ema_model:
            client_state[
                'generator_ema_model_state'] = config.generator_ema_model.state_dict(
                )
            client_state[
                'discriminator_ema_model_state'] = config.discriminator_ema_model.state_dict(
                )
